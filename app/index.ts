...
async function startServer() {
  ...
  if (configManager.getConfig().seal.sui_network === 'testnet') {
    // Additional debug endpoints for testing the master key in testnet environment
    app.get('/debug/master-key', async (_, res) => {
      try {
        ...
        const hash = crypto
          .createHash('sha256')
          .update(getCurrentMasterKey().masterKeyBuffer)
          .digest('hex');
