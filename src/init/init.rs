...
fn main() {
    ...
    unsafe {
        ...
        env::set_var("PATH", "/bin:/sbin:/usr/bin:/usr/local/bin:/usr/sbin");
    }
    ...
    match Command::new("/usr/local/bin/entrypoint.sh").spawn() {
        Ok(mut child) => {
            dmesg("Spawned entrypoint.sh script".to_string());
            // Wait for the child process to finish
            match child.wait() {
                Ok(status) => dmesg(format!("entrypoint.sh exited with status: {}", status)),
                Err(e) => eprintln!("Error waiting for entrypoint.sh: {}", e),
            }
        }
        Err(e) => eprintln!("Failed to execute entrypoint.sh: {}", e),
    }
    reboot();
}
